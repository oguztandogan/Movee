...
class CreditsViewController: UIViewController {
    ...
    @IBOutlet weak var headerSeeFullBioLabel: UILabel!
    ...
    @IBAction func headerSeeFullBioAction(_ sender: Any) {
        if headerBioLabel.numberOfLines == 4 {
            ...
            headerSeeFullBioLabel.text = NSLocalizedString("See Less Bio", comment: "")
            headerView.frame.size.height += contentHeight.first ?? 0
            creditsKnownForTableView.reloadData()
        } else {
            headerSeeFullBioLabel.text = NSLocalizedString("See Full Bio", comment: "")
